...
export async function getFoldersAndFiles(prefix: string) {
    ...
    allObjects = [
      ...files.Contents.filter((content) => {
        if (
          ...
          content.Key ===
            `${prefix}/${extractRightOfSlash(content.Key!.slice(0, -1))}/`
        )
          return true;
        else if (
          !content.Key?.endsWith("/") &&
          content.Key === `${prefix}/${extractRightOfSlash(content.Key!)}`
        )
          return true;
        else return false;
      }),
    ];
    allObjects = await Promise.all(
      allObjects.map(async (content) => {
        if (content.Key?.endsWith("/"))
          ...
          const statusReq = await fetch(
            `http://localhost:8000/file_status?s3_key=${content.Key}`
          );
  ...
  return allObjects.sort((a, b) => {
    if (a.StorageClass === "DIRECTORY" && b.StorageClass !== "DIRECTORY") {
      ...
      a.StorageClass !== "DIRECTORY" &&
      b.StorageClass === "DIRECTORY"
    ) {
      return 1;
    } else {
      return 0;
...
export async function deleteFolder(
  ...
  const objectKeys = [...(files.Contents?.map((obj) => obj.Key) ?? [])];
  await Promise.all(
...
export default async function bulkDelete(
  objects: Pick<S3Response, "OriginalKey" | "StorageClass">[],
  path: string
) {
  await Promise.all(
